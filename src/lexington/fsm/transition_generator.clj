(ns ^{ :doc "Transition Map Generator"
       :author "Yannick Scherer" }
  lexington.fsm.transition-generator)
...
_
  ...
  "
  [pairs any-entity default-state]
  (loop [pairs (cons [any-entity default-state]  (reverse pairs))
         result []
         current-default nil]
    (if-not (seq pairs)
      (into {} (map vec result))
      (let [[[e s :as pair] & rst] pairs]
        (if (= any-entity e) 
          (recur rst (conj (filter (comp not #{any-entity} first) result) pair) s)
          (recur (filter (fn [[x y]] 
                           (or (not (= e x)) y)) rst)
                 (if s
...
(defn transitions->map
  ...
  [transitions any-entity default-state]
  (-> (partition 2 transitions)
    generate-transition-pairs
    (generate-transition-map any-entity default-state)))
